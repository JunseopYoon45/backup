CREATE OR REPLACE PROCEDURE DWSCM."SP_UI_BF_00_MONTHLY_CATE_SEASONAL_INDEX" 
(
	p_VER_CD 		VARCHAR2
...
BEGIN
	...
	, SA AS (
		...
		GROUP BY BASE_DATE, IH.LVL04_CD, AH.ANCESTER_CD
	)
    , PW_SA AS (
	...
	, RT AS (
		SELECT BASE_DATE + 0	AS FROM_DATE
		 ...
		 WHERE VER_CD = p_VER_CD
		 GROUP BY BASE_DATE, IH.LVL04_CD, ACCOUNT_CD
	) 
	, PW_RT AS (
		SELECT CAL.FROM_DATE AS BASE_DATE
			 , CAL.MM
