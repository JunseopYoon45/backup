CREATE OR REPLACE PROCEDURE DWSCM."SP_UI_BF_00_CATE_SEASONAL_INDEX" 
(
	p_VER_CD 		VARCHAR2
...
BEGIN
	...
	DELETE FROM TB_BF_CATE_SEASONAL_INDEX
	WHERE VER_CD = p_VER_CD;

	COMMIT;

	INSERT INTO TB_BF_CATE_SEASONAL_INDEX
	WITH CAL AS (
	...
	, SA AS (
		...
		GROUP BY DP_WK, SUBSTR(DP_WK, 5,6), YEAR, IH.LVL04_CD
	) 
	, RT AS (
		SELECT DP_WK
			 , SUBSTR(DP_WK, 5,6) AS WEEK
	...
	, SES_1 AS (
		SELECT ITEM_CD
			 , WEEK
			 , WY_AVG/53 AS SES1
			 , CASE WHEN ROUND(WY_AVG/53, 4) >= 0.14 THEN 1 ELSE 0 END OVER_14
			 , CASE WHEN ROUND(WY_AVG/53, 4) >= 0.20 THEN 1 ELSE 0 END OVER_20 
