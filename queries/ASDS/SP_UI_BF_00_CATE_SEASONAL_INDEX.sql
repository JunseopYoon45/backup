...
IS
   ...
   **************************************************************************
   * Name : SP_UI_BF_00_CATE_SEASONAL_INDEX
   * Purpose : 주간 소분류 시즌지수 생성
   * Notes :
   **************************************************************************/
...
BEGIN
	...
	, SES_1 AS (
		SELECT A.ITEM_CD
			 ...
			 , CASE WHEN WEEK BETWEEN 1 AND 13 THEN 'Q1' 
				    ...
				    ELSE 'Q4' END QUARTER
		  FROM WK_AVG A  -- 변경 부분
		INNER JOIN (SELECT CONF_CD, ATTR_01 AS CRITERIA
					  FROM TB_CM_COMM_CONFIG
					 WHERE CONF_GRP_CD = 'BF_WEEKLY_SEASONAL_CRITERIA') B
	 	   ON A.ITEM_CD = B.CONF_CD
	) 
	, WY_SUM AS (
   ...
   SELECT RAWTOHEX(SYS_GUID()) AS ID
		...
		, NULL AS MODIFY_BY
		, NULL AS MODIFY_DTTM
		, A.CRITERIA      AS WEAK_CRITERIA		
	    , NULL            AS STRONG_CRITERIA		
	  FROM ( 
	  		SELECT ITEM_CD
	  			 , WEEK
	  			 , CRITERIA
...
--  P_RT_MSG := 'MSG_0001';  --저장 되었습니다.
	...
	EXCEPTION WHEN OTHERS THEN
		...
		BEGIN
			...
			SELECT 'SP_UI_BF_00_CATE_SEASONAL_INDEX', a, b, c, SYSDATE FROM DUAL;
