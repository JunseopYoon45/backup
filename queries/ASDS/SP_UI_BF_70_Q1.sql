CREATE OR REPLACE PROCEDURE DWSCM."SP_UI_BF_70_Q1" (
...
BEGIN
	SP_UI_BF_70_Q1('BF-20230828-01', 'M20180115092627169N446701842271O', '1001659', 'N20180115092710840N520475678180O','01_POS','', '', '', 'Y', 'Y', 'Y', 'Y', pRESULT);
	SP_UI_BF_70_Q1('BF-20230828-01', 'M20180115092627169N446701842271O', '1001659', 'FE00001E54F88F3FE053DD0A10AC762B','01','', '', '', 'Y', 'Y', 'Y', 'Y', pRESULT);
	SP_UI_BF_70_Q1('BF-20230828-01', 'M20180115092627169N446701842271O', '1001659', 'N20180115092712856O251735022591O','ALL','', '', '', 'Y', 'Y', 'Y', 'Y', pRESULT);
	SP_UI_BF_70_Q1('BF-20230828-01', 'FA5FEBBCADDED90DE053DD0A10AC8DB5', 'AC03', 'N20180115092710840N520475678180O','01_POS', '', '', '', 'N', 'Y', 'Y', 'Y', pRESULT);
...
BEGIN 
	...
	/* 동적 쿼리 생성 */
	FOR i IN (SELECT MIN(DAT) BASE_DATE FROM TB_CM_CALENDAR WHERE DAT BETWEEN v_MIN_BASE_DATE AND v_MAX_BASE_DATE GROUP BY DP_WK ORDER BY 1)	
	LOOP 
		v_NUM := v_NUM + 1;
		v_SQL_DATE := v_SQL_DATE || ', ''' || i.BASE_DATE || '''' || 'AS W' || LPAD(v_NUM, 2, '0') ;
	END LOOP;
				...
				SELECT IH.LVL02_CD      AS DIV_MAX
			      ...
			      FROM (    
					SELECT I.DESC_CD        AS ITEM_CD
						 ...
						 , CASE WHEN RT.BASE_DATE < TRUNC(SYSDATE, ''IW'') THEN SUM(NVL(SA.QTY, 0))
						 		ELSE NULL END AS ACT_SALES
						 , CASE WHEN RT.BASE_DATE < TRUNC(SYSDATE, ''IW'') THEN ABS(SUM(RT.QTY) - SUM(NVL(SA.QTY, 0)))
