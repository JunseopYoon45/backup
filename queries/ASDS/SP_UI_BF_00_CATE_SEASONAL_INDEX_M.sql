...
IS
   ...
   **************************************************************************
   * Name : SP_UI_BF_00_CATE_SEASONAL_INDEX_M
   * Purpose : 월간 소분류 시즌지수 생성
   * Notes :
   **************************************************************************/
...
--		   AND REGEXP_INSTR(RT.ITEM_CD,'[^0-9]') = 0
	...
	, SES_1 AS (
		SELECT ITEM_CD
			 , MM
			 , MM_AVG/SUM_MM_AVG AS SES1   -- 변경 부분
			 , CASE WHEN ROUND(MM_AVG/SUM_MM_AVG, 4) >= p_WEAK THEN 1 ELSE 0 END OVER_WEAK -- 변경 부분
	...
	, SES_2 AS (
		SELECT ITEM_CD
			 , MM
			 , SES2
			 , CASE WHEN ROUND(SES2) >= p_STRONG THEN 1 ELSE 0 END OVER_STRONG
			 , CASE WHEN MM BETWEEN 1 AND 3 THEN 'Q1' 
				    WHEN MM BETWEEN 4 AND 6 THEN 'Q2'
				    WHEN MM BETWEEN 7 AND 9 THEN 'Q3'
				    ELSE 'Q4' END QUARTER
   ...
   SELECT RAWTOHEX(SYS_GUID()) AS ID
	...
	, TGT AS (
		SELECT DISTINCT ITEM_CD
		  FROM TB_DPD_ITEM_HIERACHY2
		 WHERE 1 = 1
		   AND ATTR_04 IN ('1', '9')
		   AND LVL04_CD IN (
							SELECT DISTINCT LVL04_CD
							  FROM TB_DPD_ITEM_HIERACHY2  A
							 INNER JOIN TGT_FCST B
								ON A.LVL05_CD = B.ITEM_CD	   
		   				   )
	)	
--	, SA AS (
--	SELECT BASE_DATE
...
--  P_RT_MSG := 'MSG_0001';  --저장 되었습니다.
	...
	EXCEPTION WHEN OTHERS THEN
		...
		BEGIN
			...
			SELECT 'SP_UI_BF_00_CATE_SEASONAL_INDEX_M', a, b, c, SYSDATE FROM DUAL;
