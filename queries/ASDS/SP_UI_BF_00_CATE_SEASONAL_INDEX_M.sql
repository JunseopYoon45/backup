CREATE OR REPLACE PROCEDURE DWSCM."SP_UI_BF_00_CATE_SEASONAL_INDEX_M"
(
	p_VER_CD 		VARCHAR2
...
BEGIN
	...
	DELETE FROM TB_BF_CATE_SEASONAL_INDEX_M
	WHERE VER_CD = p_VER_CD;

	COMMIT;

	INSERT INTO TB_BF_CATE_SEASONAL_INDEX_M
	WITH CAL AS (
    	...
    	SELECT TRUNC(CAL.FROM_DATE, 'MM') AS BASE_DATE
	 	  ...
	 	  FROM (
	 	  		SELECT BASE_DATE 	 AS FROM_DATE
					 ...
					 , SUM(QTY) AS QTY
				  FROM DWSCM.TB_CM_ACTUAL_SALES SA
				  INNER JOIN TB_DPD_ITEM_HIERACHY2 IH ON SA.ITEM_MST_ID = IH.ITEM_ID
				  INNER JOIN TGT ON IH.LVL05_CD = TGT.ITEM_CD
				  ...
				  GROUP BY BASE_DATE, IH.LVL04_CD
				) SA  
	 	  LEFT JOIN CAL ON CAL.FROM_DATE BETWEEN SA.FROM_DATE AND SA.TO_DATE 	
	 	 GROUP BY TRUNC(CAL.FROM_DATE, 'MM'), CAL.MM, CAL.YYYY, SA.ITEM_CD
	) 
	, RT AS (
		SELECT BASE_DATE
			 , CAL.YYYY	 	AS YEAR
		 ...
		 WHERE VER_CD = p_VER_CD
		   ...
		   AND REGEXP_INSTR(RT.ITEM_CD,'[^0-9]') = 0
		 GROUP BY BASE_DATE, CAL.YYYY, CAL.MM, IH.LVL04_CD
	) 
	, AGG AS (
		SELECT * FROM SA 
		UNION ALL
	...
	, SES_1 AS (
		SELECT ITEM_CD
			 , MM
			 , MY_AVG/12 AS SES1
			 , CASE WHEN ROUND(MY_AVG/12, 4) >= 0.14 THEN 1 ELSE 0 END OVER_14
			 , CASE WHEN ROUND(MY_AVG/12, 4) >= 0.20 THEN 1 ELSE 0 END OVER_20 
	...
	, SES_2 AS (
		SELECT A.ITEM_CD, MM, CASE WHEN MY_SUM = 0 THEN 0 ELSE MY_AVG / MY_SUM * 12 END SES2
		FROM MY_AVG A
   ...
   SELECT RAWTOHEX(SYS_GUID()) AS ID
		...
		, CASE WHEN Q1_OVER_14 >= 1 THEN 1 ELSE 0 END Q1_OVER_14
		, CASE WHEN Q2_OVER_14 >= 1 THEN 1 ELSE 0 END Q2_OVER_14
		, CASE WHEN Q3_OVER_14 >= 1 THEN 1 ELSE 0 END Q3_OVER_14
		, CASE WHEN Q4_OVER_14 >= 1 THEN 1 ELSE 0 END Q4_OVER_14
		, CASE WHEN NVL(Q1_OVER_14, 0) + NVL(Q2_OVER_14, 0) + NVL(Q3_OVER_14, 0) + NVL(Q4_OVER_14, 0) >= 1 THEN 1 ELSE 0 END RES_OVER_14
		, CASE WHEN Q1_OVER_20 >= 1 THEN 1 ELSE 0 END Q1_OVER_20
		, CASE WHEN Q2_OVER_20 >= 1 THEN 1 ELSE 0 END Q2_OVER_20
		, CASE WHEN Q3_OVER_20 >= 1 THEN 1 ELSE 0 END Q3_OVER_20
		, CASE WHEN Q4_OVER_20 >= 1 THEN 1 ELSE 0 END Q4_OVER_20
		, CASE WHEN NVL(Q1_OVER_20, 0) + NVL(Q2_OVER_20, 0) + NVL(Q3_OVER_20, 0) + NVL(Q4_OVER_20, 0) >= 1 THEN 1 ELSE 0 END RES_OVER_20
		, 'SYSTEM' AS CREATE_BY
	 ...
	 INNER JOIN 
	 	   (
	 		SELECT ITEM_CD
	 			 , QUARTER
	 			 , SUM(OVER_14) AS OVER_14
	 		FROM SES_1
	 		GROUP BY ITEM_CD, QUARTER
	 	   )
	 	   PIVOT ( SUM(OVER_14) FOR QUARTER IN ('Q1' AS Q1_OVER_14, 'Q2' AS Q2_OVER_14, 'Q3' AS Q3_OVER_14, 'Q4' AS Q4_OVER_14) ) C
	 ON A.ITEM_CD = C.ITEM_CD
	 INNER JOIN 
	 	   (
	 		SELECT ITEM_CD
	 			 , QUARTER
	 			 , SUM(OVER_20) AS OVER_20
	 		FROM SES_1
	 		GROUP BY ITEM_CD, QUARTER
	 	   )
	 	   PIVOT ( SUM(OVER_20) FOR QUARTER IN ('Q1' AS Q1_OVER_20, 'Q2' AS Q2_OVER_20, 'Q3' AS Q3_OVER_20, 'Q4' AS Q4_OVER_20) ) D
