CREATE OR REPLACE PROCEDURE DWSCM."SP_UI_BF_00_AGG_CATE" (
	p_VER_CD         	VARCHAR2
...
BEGIN
	...
	DELETE FROM DWSCM.TB_BF_CATE_STATS
     WHERE VER_CD = p_VER_CD;
	COMMIT;
	INSERT INTO DWSCM.TB_BF_CATE_STATS (
		  VER_CD
		, CATE_CD
		, LOCAT_CD
		, QTY
...
--	     , CASE WHEN B.QTY_PCT_RANK >=0.8 THEN 'A'
--	            WHEN B.QTY_PCT_RANK >=0.49 THEN 'B'
--	            ELSE 'C' END AS QTY_ABC
--	     , CASE WHEN B.CV_QTY < 0.5 THEN 'X'
--	            WHEN B.CV_QTY < 1.2 THEN 'Y'
--	            ELSE 'Z' END QTY_XYZ
...
--	     , CASE WHEN B.AMT_PCT_RANK >=0.8 THEN 'A'
--	            WHEN B.AMT_PCT_RANK >=0.49 THEN 'B'
--	            ELSE 'C' END AS AMT_ABC
--	     , CASE WHEN B.CV_AMT < 0.5 THEN 'X'
--	            WHEN B.CV_AMT < 1.2 THEN 'Y'
--	            ELSE 'Z' END AMT_XYZ
	        ...
	        SELECT A.VER_CD
	          ...
	          FROM (
	                SELECT T.VER_CD
	                     ...
	                     , COUNT(*) AS CAL_CNT
	                  FROM STT T
	                 GROUP BY T.VER_CD, T.CATE_CD, T.LOCAT_CD
	                ) A
	        ) B
	;
	COMMIT;
	MERGE INTO TB_BF_CATE_STATS A USING (
	WITH IA AS (
	...
	, IA2 AS (
		SELECT IH.LVL04_CD AS ITEM_CD, SUBSTR(ACCOUNT_CD, 1,2) AS ACCOUNT_CD, ENGINE_TP_CD
		FROM IA
		INNER JOIN TB_DPD_ITEM_HIERACHY2 IH ON IA.ITEM_CD = IH.LVL05_CD
		GROUP BY IH.LVL04_CD, SUBSTR(ACCOUNT_CD, 1,2), ENGINE_TP_CD
	)
	, RT AS (
		SELECT ENGINE_TP_CD
		...
		INNER JOIN TB_DPD_ITEM_HIERACHY2 IH ON H.ITEM_CD = IH.LVL05_CD
		WHERE 1=1
		  AND BASE_DATE BETWEEN p_TARGET_FROM_DATE AND p_TARGET_TO_DATE
		  AND REGEXP_INSTR(H.ITEM_CD,'[^0-9]') = 0
		GROUP BY ENGINE_TP_CD, IH.LVL04_CD, SUBSTR(H.ACCOUNT_CD, 1,2), BASE_DATE
	)
--	, RT AS (
--			SELECT ENGINE_TP_CD
--				 , IH.LVL04_CD AS ITEM_CD
...
--									   ORDER BY 1 DESC)
--								WHERE ROWNUM < 5)
--			 GROUP BY ENGINE_TP_CD, IH.LVL04_CD, SUBSTR(RT.ACCOUNT_CD, 1,2), BASE_DATE
--	)
	, ACT_SA AS (
		SELECT TGT.ITEM_CD
			 , TGT.ACCOUNT_CD
	...
	, SA AS (
		SELECT IH.LVL04_CD AS ITEM_CD
			 ...
			 , SUM(QTY) AS QTY
		FROM ACT_SA SA
		INNER JOIN TB_DPD_ITEM_HIERACHY2 IH ON SA.ITEM_CD = IH.LVL05_CD
		GROUP BY IH.LVL04_CD, SUBSTR(ACCOUNT_CD, 1,2), BASE_DATE
	) 
	, WAPE AS (
	...
	SELECT CATE_CD, LOCAT_CD, ACCRCY, VER_CD FROM FINAL) B 
	ON (A.CATE_CD = B.CATE_CD AND A.LOCAT_CD = B.LOCAT_CD AND A.VER_CD = B.VER_CD)
	WHEN MATCHED THEN 
	UPDATE SET A.ACCRCY = B.ACCRCY
	WHEN NOT MATCHED THEN
	INSERT (CATE_CD, LOCAT_CD, ACCRCY, VER_CD) VALUES (B.CATE_CD, B.LOCAT_CD, B.ACCRCY, B.VER_CD)
    ;
	COMMIT;
