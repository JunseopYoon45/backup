...
--	            ELSE 'Z' END AMT_XYZ
		...
		SELECT A.ENGINE_TP_CD 	   
		  ...
		  FROM (
	            SELECT RT.ITEM_CD
	              ...
	              FROM RT
	             INNER JOIN SA ON RT.ITEM_CD = SA.ITEM_CD AND RT.ACCOUNT_CD = SA.ACCOUNT_CD AND RT.BASE_DATE = SA.BASE_DATE
		        ) A
	...
	SELECT CATE_CD, LOCAT_CD, ACCRCY FROM FINAL) B 
	ON (A.CATE_CD = B.CATE_CD AND A.LOCAT_CD = B.LOCAT_CD)
	WHEN MATCHED THEN 
	UPDATE SET A.ACCRCY = B.ACCRCY
	WHEN NOT MATCHED THEN
	INSERT (CATE_CD, LOCAT_CD, ACCRCY) VALUES (B.CATE_CD, B.LOCAT_CD, B.ACCRCY)
    ;
